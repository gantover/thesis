...
def fid(image_path, ref_path, num_expected, seed=0, batch=64, idx_path=None, fid_features=None, clip_features=None):
    """Calculate FID for a given set of images."""
    try:
        torch.multiprocessing.set_start_method("spawn")
    except RuntimeError:
    ...
    dist.print0("Calculating FID...")
    if dist.get_rank() == 0:
        fid_val = calculate_fid_from_inception_stats(mu, sigma, ref["mu"], ref["sigma"])
        print(f"{fid_val:g}")
        return fid_val
    torch.distributed.barrier()
    return None
