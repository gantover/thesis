...
def precision_recall(ref_features_path, eval_features_path, realism_path=None, rarity_path=None, row_batch_size=2000, col_batch_size=2000):
    ref_features = torch.load(ref_features_path).cpu().numpy()
    eval_features = torch.load(eval_features_path).cpu().numpy()
    ...
    # Calculate k-NN precision and recall.
    if realism_path is not None:
        state, realism = knn_precision_recall_features(ref_features, eval_features, realism=True, row_batch_size=row_batch_size, col_batch_size=col_batch_size)
        if ".npy" in realism_path:
            _ensure_parent_dir(realism_path)
            np.save(realism_path, realism)
            print(f"Realism score saved to {realism_path}")
        ...
        state = knn_precision_recall_features(ref_features, eval_features, realism=False, row_batch_size=row_batch_size, col_batch_size=col_batch_size)

    # Compute rarity scores if requested (independent of realism).
    if rarity_path is not None:
        rarity = compute_rarity_scores(ref_features, eval_features, row_batch_size=row_batch_size, col_batch_size=col_batch_size)
        if ".npy" in rarity_path:
            _ensure_parent_dir(rarity_path)
            np.save(rarity_path, rarity)
            print(f"Rarity score saved to {rarity_path}")
