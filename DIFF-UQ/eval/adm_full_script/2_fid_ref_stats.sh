...
if [ ! -f $ROOT_PATH/fid-refs/imagenet-train-128x128.zip ]; then
    python dataset_tool.py \
        --source=${TRAIN_DATA_PATH} \
        --dest=$ROOT_PATH/fid-refs/imagenet-train-128x128.zip \
        --resolution=128x128 \
        --transform=center-crop \
        --max-images 50000
fi
python fid.py ref \
    --data=$ROOT_PATH/fid-refs/imagenet-train-128x128.zip \
    --dest=$ROOT_PATH/fid-refs/imagenet-train-128x128.npz \
    --fid_features=$ROOT_PATH/precision-recall-refs/image_net_train_${H}_fid_features_.pt